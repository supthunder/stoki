...
export async function GET(request: Request) {
  ...
  try {
    console.log('Starting portfolio data update cron job...');
      ...
      try {
        ...
        for (let i = 0; i < symbols.length; i += batchSize) {
          ...
          try {
            ...
            console.error(`Error fetching batch of quotes:`, err);
          }
        ...
        await cacheData('stock:prices:current', pricesObject, ONE_HOUR_IN_SECONDS); // 1 hour
    ...
    if (allSymbols.size > 0) {
      try {
        ...
        await cacheData('stock:prices:1day', prices1DayObject, ONE_DAY_IN_SECONDS); // 24 hours
        await cacheData('stock:prices:7days', prices7DaysObject, ONE_DAY_IN_SECONDS); // 24 hours
      ...
      try {
        await sql`
          INSERT INTO portfolio_summaries (
            ...
            ${user.id}, ${totalCurrentValue}, ${totalPurchaseValue}, ${totalGain}, ${totalGainPercentage},
            ${dailyGain}, ${dailyGainPercentage}, ${weeklyGain}, ${weeklyGainPercentage}, NOW()
          )
    ...
    await cacheData('portfolio_data:last_updated', new Date().toISOString(), ONE_DAY_IN_SECONDS);
    
    // Also cache pre-formatted leaderboard data for each time frame
    await cacheLeaderboardData(sql, 'total');
    await cacheLeaderboardData(sql, 'daily');
    await cacheLeaderboardData(sql, 'weekly');
    ...
    return NextResponse.json({ 
      ...
      symbolsProcessed: allSymbols.size
    });
  } catch (error) {
...
async function cacheLeaderboardData(sql: any, timeFrame: string) {
  try {
    ...
    await cacheData(cacheKey, formattedResults, ONE_HOUR_IN_SECONDS);
